...
class Dataset(data.Dataset):
    ...
    def load_label(filenames):
        print("filenames[0]: ", filenames[0], os.path.exists(filenames[0]))
                ...
                if os.path.isfile(b.join(filename.rsplit(a, 1)).rsplit('.', 1)[0] + '.txt'):
                    ...
                    if nl:
                        ...
                        if not inside_mask.all():
                            outside_boxes = label[~inside_mask]
                            ob += len(outside_boxes)
                            
                            for idx, box in enumerate(outside_boxes):
                                cls, xc, yc, bw, bh = box
                                #print(f"  Class {int(cls)} | x_center: {xc:.3f}, y_center: {yc:.3f}, w: {bw:.3f}, h: {bh:.3f}")
                                #print(f"    → x_range: [{xc - bw/2:.3f}, {xc + bw/2:.3f}], y_range: [{yc - bh/2:.3f}, {yc + bh/2:.3f}]")
